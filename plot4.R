...
subelecdata$dtcomb<-strptime(paste(as.character(subelecdata$Date), as.character(subelecdata$Time))
                 ,"%d/%m/%Y %H:%M:%S") 
#dtstrp<-strptime(dtcomb, "%d/%m/%Y %H:%M:%S")
...
#legend("topright",cex=.6,merge=TRUE, y.intersp=.8, c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
  #     , lty=c(1,1,1), col = c("black", "red", "blue"))


legend("topright",cex=.6, c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
     , lty=c(1,1,1), col = c("black", "red", "blue"))
...
legend("topright",bty="n", cex=.5, c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
       , lty=c(1,1,1), col = c("black", "red", "blue"))
# Panel 4
